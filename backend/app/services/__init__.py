...
class UserService:
    ...
    def register_user(self, data: dict):
        ...
        if data["role"] not in ("buyer", "seller"):
            raise ValueError("Role deve ser 'buyer' ou 'seller'")
    ...
    def get_user(self, user_id: int):
        ...
        if not user:
            raise LookupError(f"Usuario {user_id} nao encontrado")
        return user
...
class ProductService:
    ...
    def create_product(self, data: dict):
        ...
        if not seller:
            raise LookupError("Vendedor nao encontrado")
        if seller.role != "seller":
            raise ValueError("Usuario nao e um vendedor")
    ...
    def list_products_by_seller(self, seller_id: int):
        seller = self.user_repo.find_by_id(seller_id)
        if not seller:
            raise LookupError("Vendedor nao encontrado")
        return self.repo.find_by_seller(seller_id)

    def update_product(self, product_id: int, data: dict):
        product = self.get_product(product_id)
        allowed_fields = ["title", "description", "price", "stock", "status"]
        valid_statuses = {"active", "sold_out", "inactive"}
...
class OrderService:
    ...
    def create_order(self, data: dict):
        ...
        if quantity < 1:
            raise ValueError("Quantidade minima e 1")

        buyer = self.user_repo.find_by_id(data["buyer_id"])
        if not buyer:
            raise LookupError("Comprador nao encontrado")
        if buyer.role != "buyer":
            raise ValueError("Usuario nao e um comprador")

        product = self.product_repo.find_by_id(data["product_id"])
        if not product:
            raise LookupError("Produto nao encontrado")
        if product.status != "active":
            raise ValueError("Produto nao disponivel para compra")
        if product.stock < quantity:
            raise ValueError(f"Estoque insuficiente. Disponivel: {product.stock}")
        ...
        self.product_repo.update_stock(product, quantity)
    ...
    def get_order(self, order_id: int):
        ...
        if not order:
            raise LookupError(f"Pedido {order_id} nao encontrado")
        return order

    def list_orders_by_buyer(self, buyer_id: int):
        buyer = self.user_repo.find_by_id(buyer_id)
        if not buyer:
            raise LookupError("Comprador nao encontrado")
        return self.repo.find_by_buyer(buyer_id)
